...
def _write_section_lines(path, data, section_header, line_format):
    """Write lines of a specified section to a specified file.
    ...
    with open(path, 'a') as f:
      ...
      f.write("".join(
        line_format.format(**dic)
        for dic in (dict(zip(keys, t)) for t in zip(*values))))
...
class LammpsAtoms:
  ...
  def __init__(
    ...
    velocities: None or array-like object of float
      Something like returned value of ``ase.Atoms.get_velocities()``.
  ...
  def get_required_datanames(self, molecule=False):
    ...
    Names of data are returned if the data is required
    to write Lammps' data file and has not been set yet.
  ...
  def write_lines_for_molecule(self, path, mass=False, **kwargs):
    """Write lines of *Coords*, *Types*, *Charges* and *Masses* section
...
class LammpsTopology:
  ...
  def set_types(self, seq_to_type, atom_types):
    ...
    self._set_data(type=[
      full_dict[seq] for seq in (
        tuple(atom_types[i-1] for i in seq) for seq in self._sequences)])

  def write_lines(self, path):
    """Write lines of a topology section
    in Lammps' data file (or molecule file).
  ...
  def _make_full_seq_to_type(self, seq_to_type):
    """Return *full* mapping from sequences of atom types
...
class LammpsSpecialBonds:
  ...
  def write_lines(self, path):
    """Write lines of *Special Bond Counts*
    and *Special Bonds* section in Lammps' molecule file.
